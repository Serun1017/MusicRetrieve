    ...
    def __init__(self, args, type='origin') :
        ...
        if type == 'origin' :
            self.seq = nn.Sequential(
                ...
                nn.Conv1d(in_channels=1, out_channels=64, kernel_size=args.audio_sampling_rate*10, stride=int(args.audio_sampling_rate*0.25*0.25), padding=int(args.audio_sampling_rate*5) - 1, padding_mode='zeros', bias=False), # [3840] = each token contains 10 sec sample of audio, stride means 0.0625 sec
                nn.ReLU(),
            ...
            self.seq = nn.Sequential(
                ...
                nn.Conv1d(1, 256, args.audio_sampling_rate*1, int(args.audio_sampling_rate*0.25*0.25), int(args.audio_sampling_rate*0.5), padding_mode='zeros', bias=False), # [160] = each token means 1 sec, stride means 0.0625 sec
                nn.ReLU(),
